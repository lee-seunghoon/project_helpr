...
from .models import User, xyLocation, Message, monthChat
...
import pymysql
import datetime
# from .weatherfun import MaxMinTemp, MaxMinTemp, currentTemp, currentWeather
...
def chart(request):
    if request.method == "POST":
        chatDate = request.POST.get('formdate')
        print(chatDate)
        
        try: 
            chatData_list = monthChat.objects.filter(conversationDate=chatDate)
            content_chat = {'datas':chatData_list,
                            'focus': "T"}
            return render(request, 'chart.html', content_chat)
        except:
            chatData_list = monthChat.objects.filter(conversationDate="2021-05-15")
            content_chat = {'datas':chatData_list,
                            'focus':"F"}
            return render(request, 'chart.html', content_chat)
        # 회원인 경우
...
def map(request):
    conn = pymysql.connect(host='13.209.193.138', port=3306, user='DBuser', passwd='123456', db='mariaDB1', charset="utf8")
    try:
        cur = conn.cursor()
    ...
    MaxTemp = MaxMinTemp(user.region_1,user.region_2,user.region_3, 'MAX')
    MinTemp = MaxMinTemp(user.region_1,user.region_2,user.region_3, 'MIN')
    cTemp = currentTemp(user.region_1,user.region_2,user.region_3)
    cWeather = currentWeather(user.region_1,user.region_2,user.region_3)
    today = datetime.datetime.today()  # 오늘의 날씨를 알려주는 함수
    current_date = today.strftime("%Y년 %m월 %d일")
    current_time = today.strftime("%H시 %M분")
...
def msg(request):
    ...
    if request.method == "POST":
        ...
        message.save()
        # mqtt로 pub
        tts = gTTS(text=m_content, lang='ko')
        tts.save('../message/test.wav')
        topic = "tts/message"
